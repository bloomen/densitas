...
void assign_vector_to_row(MatrixType& matrix, size_t row_index, const VectorType& vector)
{
    ...
    const auto n_rows = densitas::matrix_adaptor::n_rows(matrix);
    ...
    const auto n_cols = densitas::matrix_adaptor::n_columns(matrix);
    if (n_cols != n_elem)
    ...
    for (size_t i=0; i<n_cols; ++i) {
        ...
        densitas::matrix_adaptor::set_element<ElementType>(matrix, row_index, i, value);
    }
}
...
VectorType extract_row(const MatrixType& matrix, size_t row_index)
{
    ...
    if (row_index > n_rows-1)
        throw densitas::densitas_error("row index larger than rows in matrix: " + std::to_string(row_index));
    const auto n_cols = densitas::matrix_adaptor::n_columns(matrix);
    auto vector = densitas::vector_adaptor::construct_uninitialized<VectorType>(n_cols);
    for (size_t i=0; i<n_cols; ++i) {
        const auto value = densitas::matrix_adaptor::get_element<ElementType>(matrix, row_index, i);
        densitas::vector_adaptor::set_element<ElementType>(vector, i, value);
    }
    return vector;
}
...
{
    const auto n_cols = densitas::matrix_adaptor::n_columns(X);
    const auto feature_row = densitas::core::extract_row<ElementType, VectorType>(X, row_index);
    auto feature_matrix = densitas::matrix_adaptor::construct_uninitialized<MatrixType>(1, n_cols);
    densitas::core::assign_vector_to_row<ElementType>(feature_matrix, 0, feature_row);
    const auto prob_pred = densitas::model_adaptor::predict_proba<VectorType>(model, feature_matrix);
    return densitas::vector_adaptor::get_element<ElementType>(prob_pred, 0);
}
