...
VectorType make_classification_target(const VectorType& y, ElementType lower, ElementType upper)
{
    ...
    const auto n_elem = densitas::vector_adaptor::n_elements(y);
    auto target = densitas::vector_adaptor::construct_uninitialized<VectorType>(n_elem);
    for (size_t i=0; i<n_elem; ++i) {
        const auto value = densitas::vector_adaptor::get_element<ElementType>(y, i);
        const auto cls_result = value<lower || value>upper ? densitas::model_adaptor::no<ModelType>() : densitas::model_adaptor::yes<ModelType>();
        densitas::vector_adaptor::set_element<ElementType>(target, i, cls_result);
    }
    return target;
}
...
ElementType minimum(const VectorType& vector)
{
    ...
    for (size_t i=0; i<n_elem; ++i) {
        const auto value = densitas::vector_adaptor::get_element<ElementType>(vector, i);
        if (value < minimum)
            minimum = value;
    }
    return minimum;
...
VectorType quantiles(const VectorType& vector, const VectorType& probas)
{
    ...
    for (size_t i=0; i<n_elem; ++i) {
        data[i] = densitas::vector_adaptor::get_element<ElementType>(vector, i);
    }
    const auto n_probas = densitas::vector_adaptor::n_elements(probas);
    auto quantiles = densitas::vector_adaptor::construct_uninitialized<VectorType>(n_probas);
    for (size_t i=0; i<n_probas; ++i) {
        const auto proba = densitas::vector_adaptor::get_element<ElementType>(probas, i);
        const auto quantile = densitas::math::quantile(data, proba);
        densitas::vector_adaptor::set_element<ElementType>(quantiles, i, quantile);
    }
    return quantiles;
}
...
VectorType quantiles_weighted(const VectorType& vector, const VectorType& weights, const VectorType& probas, ElementType accuracy)
{
    densitas::core::check_element_type<ElementType>();
    const auto n_elem = densitas::vector_adaptor::n_elements(vector);
    if (n_elem != densitas::vector_adaptor::n_elements(weights))
    ...
    for (size_t i=0; i<n_elem; ++i) {
        const auto weight = densitas::vector_adaptor::get_element<ElementType>(weights, i);
        counts[i] = static_cast<size_t>(weight / min_weight);
    }
    const auto n_vals = std::accumulate(counts.begin(), counts.end(), 0);
    auto extended = n_vals > 0 ? densitas::vector_adaptor::construct_uninitialized<VectorType>(n_vals) : vector;
    if (n_vals > 0) {
        size_t index = 0;
        for (size_t i=0; i<counts.size(); ++i) {
            const auto value = densitas::vector_adaptor::get_element<ElementType>(vector, i);
            for (size_t j=0; j<counts[i]; ++j) {
                densitas::vector_adaptor::set_element<ElementType>(extended, index, value);
                ++index;
            }
        }
    }
